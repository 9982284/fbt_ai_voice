...
FbtVoiceTransport::FbtVoiceTransport(EventGroupHandle_t event_group, FbtAudioRepeater *audio_repeater, FbtMqttServer *mqtt_server)
    : board_(Board::GetInstance()),
      ...
      voice_ui_(FbtUiVoice::GetInstance()) {
    ...
    event_bus.Subscribe("fbt_voice", FbtEvents::PHONE_STARTED, [this](const std::string &type, const std::string &data) {
        if (rtc_state_ != kIdle) {
            close_on_timeout();
        }
        rtc_state_ = kUnavailable;
        // voice_ui_.HideEnterButton();
    });

    event_bus.Subscribe("fbt_voice", FbtEvents::PHONE_CLOSED, [this](const std::string &type, const std::string &data) {
        rtc_state_ = kIdle;
        voice_ui_.EnterIdle();
    });
...
bool FbtVoiceTransport::Start(std::string data) {
    ...
    if (!open_server()) {
        ESP_LOGE(TAG, "Failed to open voice server");
        return false;
    }
    voice_ui_.Initialize();
    return true;
}

bool FbtVoiceTransport::open_server() {
    // 1. 参数检查（修正逻辑错误）
...
void FbtVoiceTransport::on_udp_packet(const std::string &payload) {
    if (payload.empty() || !is_running_ || rtc_state_ == kUnavailable) {
        return;
    }
...
void FbtVoiceTransport::start_tune_in(const std::string &payload) {
    if (rtc_state_ == kSpeaking || rtc_state_ == kTuneIn) {
        return;
    };
...
bool FbtVoiceTransport::OnEnterVoiceMode() {
    ...
    is_enter_ = true;
    voice_ui_.SetEnterState(is_enter_);
    display_->SetChatMessage("system", "");
    return start_voice();
}
...
bool FbtVoiceTransport::is_unavailable() {
    return !udp_ || group_id_.empty() || device_id_.empty() || !is_running_ || rtc_state_ == kUnavailable;
}
