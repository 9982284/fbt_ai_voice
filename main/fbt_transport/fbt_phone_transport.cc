...
void FbtPhoneTransport::onStartPhone(const std::string &json_message) {
    ...
    audio_repeater_->PlayRingtone([this]() -> bool {
        return this->rtc_state_ == kPhoneRing || this->rtc_state_ == kOffer;
    });
...
void FbtPhoneTransport::ClosePhone() {
    if (!is_running_ || rtc_state_ == kIdle) {
        return;
    }
    send_bye();
    close();
...
bool FbtPhoneTransport::parse_config(const std::string &json_message) {
    ...
    if (cJSON_IsNumber(callType)) {
        session_.callType = static_cast<FbtCallType>(callType->valueint);
    }
...
bool FbtPhoneTransport::send_udp_message(const std::string &type) {
    ...
    bool result = udp_->Send(packet) > 0;
    return result;
}
...
void FbtPhoneTransport::handle_call_out() {
    ...
    display_->SetStatus(Lang::Strings::FBT_PHONE_RINGING);
}

void FbtPhoneTransport::handle_call_in() {
    rtc_state_ = kPhoneRing;
    display_->SetStatus(Lang::Strings::FBT_PHONE_AUDIO_IN);
}
...
void FbtPhoneTransport::start_call() {
    ...
    rtc_state_ = kActive;
    display_->SetStatus(Lang::Strings::FBT_PHONE_STARTED);
    display_->SetChatMessage("sysmte", session_.name.c_str());
}
